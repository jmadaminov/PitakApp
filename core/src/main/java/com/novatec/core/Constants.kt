package com.novatec.core

/**
 * Created by jahon on 06-May-20
 */
object Constants {
    //    http://178.218.207.92:9091/api/v1/force_update/action
//    const val BASE_URL = "http://178.218.207.92:9091/api/v1/"
    const val BASE_URL = "https://api.epitak.uz/api/v1/"

    //    const val BASE_URL = "http://codeuz.uz:9091/api/v1/"
    const val MAP_FILE_BASE_URL: String = "http://tt.sablab.org/"

    const val mapZIPFilesLocation = "/graphhopper/maps/uzbekistan-latest-gh.zip"

    //    const val mapZIPFilesLocation = "/uzbekistan-latest-gh.zip"
    const val mapFilesLocation = "/graphhopper/maps/"

    //    const val mapFilesLocation = "/"
    val uzbMapFilesLocation: String = "/graphhopper/maps/uzbekistan-latest-gh/"

    const val ROLE_DRIVER = "DRIVER"
    const val errPhoneFormat = -99
    const val TXT_CAR = "CAR"
    const val TXT_DRIVER_POST = "DRIVER_POST"
//    const val FUEL_TYPE_PROPANE = "PROPANE"
//    const val FUEL_TYPE_METHANE = "METHANE"
//    const val FUEL_TYPE_PETROL = "PETROL"
    const val TXT_UDID = "UDID"
    const val TXT_PHONE_NUMBER = "PHONE_NUMBER"
    const val RU = "ru"
    const val EN = "en"
    const val UZ = "uz"
    const val CODE_ADD_CAR = 99

}